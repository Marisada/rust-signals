...
fn test_truncate() {
    ...
    is_eq(vec![], vec![], |v| {let _ = v.truncate(10);}, vec![
        Poll::Pending,
        Poll::Ready(None),
    ]);

    is_eq(vec![5, 10, 15], vec![5, 10, 15], |v| {let _ = v.truncate(3);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![5, 10, 15] } )),
        Poll::Pending,
        Poll::Ready(None),
    ]);

    is_eq(vec![5, 10, 15], vec![5, 10, 15], |v| {let _ = v.truncate(10);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![5, 10, 15] } )),
        Poll::Pending,
        Poll::Ready(None),
    ]);

    is_eq(vec![5, 10, 15], vec![5, 10], |v| {let _ = v.truncate(2);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![5, 10, 15] } )),
        Poll::Pending,
        Poll::Ready(Some(VecDiff::Pop {})),
        Poll::Ready(None),
    ...
    is_eq(vec![5, 10, 15], vec![5], |v| {let _ = v.truncate(1);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![5, 10, 15] } )),
        Poll::Pending,
        Poll::Ready(Some(VecDiff::Pop {})),
        Poll::Ready(Some(VecDiff::Pop {})),
    ...
    is_eq(vec![5, 10, 15], vec![], |v| {let _ = v.truncate(0);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![5, 10, 15] } )),
        Poll::Pending,
        Poll::Ready(Some(VecDiff::Clear {})),
        Poll::Ready(None),
...
#[test]
fn test_extend() {
    is_eq(vec![], vec![], |v| { let _ = v.extend(0..0);}, vec![
        Poll::Pending,
        Poll::Ready(None),
    ]);

    is_eq(vec![], vec![0, 1, 2], |v| { let _ = v.extend(0..3);}, vec![
        Poll::Pending,
    ...
    is_eq(vec![0, 1, 2], vec![0, 1, 2, 3, 4, 5], |v| { let _ = v.extend(3..6);}, vec![
    ...
    is_eq(vec![0, 1, 2], vec![0, 1, 2], |v| { let _ = v.extend(0..0);}, vec![
        Poll::Ready(Some(VecDiff::Replace { values: vec![0, 1, 2] } )),
        Poll::Pending,
        Poll::Ready(None),
    ]);
